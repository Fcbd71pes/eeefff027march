# bot.py - Mini App API Integrated Version
...
# --- Keyboards (Mini App Button Added) ---
# ⚠️ নিচের লিংকের জায়গায় আপনার Netlify লিংকটি বসান!
WEB_APP_URL = "https://agent-699c980c7d3b8a5e3--majestic-strudel-350c8a.netlify.app/"

MAIN_KEYBOARD = ReplyKeyboardMarkup([
    [KeyboardButton("📱 Open Mini App", web_app=WebAppInfo(url=WEB_APP_URL))],
    ["🎮 Play 1v1 (Classic)", "💰 My Wallet"], 
    ["📋 Profile", "📜 Rules"], 
...
# ==========================================
# 🚀 API RECEIVER: MINI APP DATA HANDLER
# ==========================================
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """এই ফাংশনটি মিনি অ্যাপ থেকে আসা সিগন্যাল (JSON Data) রিসিভ করে।"""
    try:
        data_str = update.effective_message.web_app_data.data
        data = json.loads(data_str)
        user_id = update.effective_user.id
        
        # যদি মিনি অ্যাপ থেকে Find Match এ ক্লিক করা হয়
        if data.get('action') == 'find_match':
            fee = float(data.get('fee', 0))
            player1 = await db.get_user(user_id)
            
            if not player1 or not await check_channel_member(update, context) or not player1.get('is_registered'): 
                return await update.message.reply_text("ম্যাচ খেলার আগে /start করে রেজিস্ট্রেশন করুন ও চ্যানেলে যোগ দিন।", reply_markup=MAIN_KEYBOARD)
            
            if fee > 0 and player1['balance'] < fee: 
                return await update.message.reply_text('❌ আপনার অ্যাকাউন্টে অপর্যাপ্ত ব্যালেন্স। দয়া করে ডিপোজিট করুন।', reply_markup=MAIN_KEYBOARD)
                
            if await db.get_from_queue(user_id): 
                return await update.message.reply_text("⏳ আপনি ইতিমধ্যে একটি ম্যাচ খুঁজছেন।", reply_markup=MAIN_KEYBOARD)
                
            async with db._lock:
                opponent = await db.find_opponent_in_queue(fee, user_id)
                
                # প্রতিপক্ষ পাওয়া গেলে!
                if opponent:
                    player2_id = opponent['user_id']
                    await db.remove_from_queue(player2_id, refund=False)
                    match_id = await db.create_match(user_id, player2_id, fee, deduct_p1=True)
                    ...
                    p1_msg = f"🔥 <b>প্রতিপক্ষ পাওয়া গেছে!</b> 🔥\nআপনার ম্যাচ {esc(player2.get('ingame_name'))} এর সাথে।\n\nঅনুগ্রহ করে eFootball গেমে Friend Match রুম তৈরি করে <b>রুম কোডটি এখানে পাঠান</b>।"
                    p2_msg = f"🔥 <b>প্রতিপক্ষ পাওয়া গেছে!</b> 🔥\nআপনার ম্যাচ {esc(player1.get('ingame_name'))} এর সাথে। রুম কোডের জন্য অপেক্ষা করুন।"
                    
                    await context.bot.send_message(user_id, p1_msg, reply_markup=CANCEL_KEYBOARD, parse_mode='HTML')
                    await db.set_user_state(user_id, 'awaiting_room_code', match_id)
                    await context.bot.send_message(player2_id, p2_msg, parse_mode='HTML')
                    
                # প্রতিপক্ষ না থাকলে লবিতে পোস্ট করবে
                else:
                    fee_text = f"<b>এন্ট্রি ফি:</b> {fee:.2f} TK" if fee > 0 else "<b>ধরন:</b> Fun Match (Free)"
                    lobby_text = f"🔥 <b>নতুন চ্যালেঞ্জ! (Mini App)</b> 🔥\n\n<b>প্লেয়ার:</b> {esc(player1.get('ingame_name'))} (ELO: {player1.get('elo_rating', 1000)})\n{fee_text}"
                    try:
                        lobby_message = await context.bot.send_message(config.LOBBY_CHANNEL_ID, lobby_text, parse_mode='HTML')
                        await db.add_to_queue(user_id, fee, lobby_message.message_id)
                        kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ বাতিল করুন", callback_data=f"cancel_{user_id}")]])
                        await update.message.reply_text(f"⏳ {fee} TK এর ম্যাচ খোঁজা হচ্ছে...\nআপনার চ্যালেঞ্জটি লবিতে পোস্ট করা হয়েছে। প্রতিপক্ষ পাওয়া গেলে আপনাকে নোটিফিকেশন দেওয়া হবে।", reply_markup=kb)
                    except Exception as e:
                        logger.error(f"Failed to post to lobby: {e}", exc_info=True)
                        await update.message.reply_text("লবিতে পোস্ট করা সম্ভব হচ্ছে না।")

    except Exception as e:
        logger.error(f"Web App Data Error: {e}", exc_info=True)
...
async def main_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "🎮 Play 1v1 (Classic)": return await play_1v1_menu(update, context)
...
async def play_1v1_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if free_play_status == 'on': kb.append([InlineKeyboardButton('🎮 Fun Match (Free)', callback_data='play_fee_0')])
    kb.extend([
...
async def backup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id not in config.ADMINS: return await update.message.reply_text("এই কমান্ডটি শুধুমাত্র অ্যাডমিনদের জন্য।")
    try: await context.bot.send_document(chat_id=user_id, document=open(config.LOCAL_DB, 'rb'), caption=f"✅ ডাটাবেস ব্যাকআপ ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
    except Exception as e: await update.message.reply_text(f"❌ একটি ত্রুটি ঘটেছে: {e}")
...
async def userinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        kb = [[InlineKeyboardButton("🔒 ব্যান করুন", callback_data=f"admin_ban_{target_user_id}")], [InlineKeyboardButton("💰 ব্যালেন্স সেট", callback_data=f"admin_setbal_{target_user_id}")]]
...
async def matchinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        p1 = await db.get_user(match['player1_id']); p2 = await db.get_user(match['player2_id'])
...
def main():
    ...
    # 🔥 Web App Handler যুক্ত করা হয়েছে 🔥
    ...
    logger.info('Bot is starting securely with Mini App support...')
    app.run_polling()
